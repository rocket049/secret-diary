...
func (ptr *QDomDocument) SetContent9(reader core.QXmlStreamReader_ITF, namespaceProcessing bool, errorMsg string, errorLine int, errorColumn int) bool {
	if ptr.Pointer() != nil {
		var errorMsgC *C.char
		if errorMsg != "" {
			errorMsgC = C.CString(errorMsg)
		...
		return int8(C.QDomDocument_SetContent9(ptr.Pointer(), core.PointerFromQXmlStreamReader(reader), C.char(int8(qt.GoBoolToInt(namespaceProcessing))), C.struct_QtXml_PackedString{data: errorMsgC, len: C.longlong(len(errorMsg))}, C.int(int32(errorLine)), C.int(int32(errorColumn)))) != 0
	}
	return false
}
